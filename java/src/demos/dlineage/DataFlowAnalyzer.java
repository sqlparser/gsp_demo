...
import demos.dlineage.dataflow.model.*;
import demos.dlineage.dataflow.model.JoinRelation.JoinClauseType;
import demos.dlineage.dataflow.model.xml.*;
...
public class DataFlowAnalyzer {
    ...
    private void analyzeCreateTableStmt(TCreateTableSqlStatement stmt) {
        ...
        if (table != null) {
            ...
            if (stmt.getSubQuery() != null
                ...
                for (int i = 0; i < resultSetModel.getColumns().size(); i++) {
                    ...
                    if (resultColumn
                        ...
                        if (alias != null && alias.getAliasName() != null) {
                            ...
                            ResultColumn column = (ResultColumn) modelManager
                                    .getModel(
                ...
                SelectSetResultSet resultSetModel = (SelectSetResultSet) modelManager
                ...
                for (int i = 0; i < resultSetModel.getColumns().size(); i++) {
                    ...
                    if (resultColumn
                        ...
                        if (alias != null && alias.getAliasName() != null) {
                            ...
                            .getColumnObject() instanceof TObjectName) {
                        TableColumn tableColumn = modelFactory.createTableColumn(
                                tableModel,
    ...
    private List<TableColumn> bindInsertTableColumn(Table tableModel,
                                                    TInsertIntoValue value, List<TObjectName> keyMap, List<TResultColumn> valueMap) {
                                            ...
                                            TObjectName columnName )
    {
        for ( int i = 0; i < resultColumns.size( ); i++ )
        {
            ...
            if ( column.getAlias( ) != null
                    && column.getAlias( ).equalsIgnoreCase(
                    columnName.getColumnNameOnly( ) ) )
                return column;
    ...
    private void analyzeInsertStmt(TInsertSqlStatement stmt) {
        ...
        if (stmt.getInsertConditions() != null
            ...
            for (int i = 0; i < stmt.getInsertConditions().size(); i++) {
                ...
                for (int j = 0; j < condition.getInsertIntoValues()
                        .size(); j++) {
                    ...
                    List<TableColumn> tableColumns = bindInsertTableColumn(
                            tableModel, value, keyMap, valueMap);
            ...
            if (table == null) {
                System.err.println(
                        "Can't handle insert statement: " + stmt.toString());
                return;
            }
            Table tableModel = modelFactory.createTable(table);
            inserTables.add(tableModel);
                ...
                if (stmt.getColumnList() != null
                    ...
                    for (int i = 0; i < items.size(); i++) {
                        ...
                        if (column != null) {
                            ...
                            if (!initColumn) {
                                tableColumn = modelFactory.createTableColumn(
                                        tableModel, column);
                    ...
                    for (int i = 0; i < resultSetModel.getColumns()
                        ...
                        if (resultColumn
                            ...
                            TableColumn tableColumn;
                            if (!initColumn) {
                                tableColumn = modelFactory
                                        .createInsertTableColumn(tableModel,
                                                (TObjectName) resultColumn
                                                        .getColumnObject());

                            } else {
                                TObjectName matchedColumnName = (TObjectName) resultColumn
                                        .getColumnObject();
                                tableColumn = matchColumn(tableColumns,
                                        matchedColumnName);
                                if (tableColumn == null) {
                            ...
                            if (alias != null
                                ...
                                relation.addSource(
                                    ...
                                    .getColumnObject())
                                    .getFieldAttr() != null) {
                                TObjectName fieldAttr = ((TResultColumn) resultColumn
                                ...
                                if (resultColumn != null
                                        ...
                                        .isEmpty()) {
                                    tableColumn
                                            .bindStarLinkColumns(resultColumn
                                                    .getStarLinkColumns());
                                }
                    ...
                    if (resultSetModel != null) {
                        for (int i = 0; i < resultSetModel.getColumns()
                                .size(); i++) {
                            ResultColumn resultColumn = resultSetModel
                                    .getColumns().get(i);
                            TAliasClause alias = ((TResultColumn) resultColumn
                                    .getColumnObject()).getAliasClause();
                            if (alias != null
                    ...
                    if (model instanceof SelectResultSet) {
                        ...
                        for (int k = 0; k < columns.size(); k++) {
                            ...
                            if (resultColumn.getAlias() != null
                                    && columnName.toString().equalsIgnoreCase(
                ...
                for (int i = 0; i < resultSetModel.getColumns().size(); i++) {
                    ...
                    if (resultColumn
                        ...
                        if (alias != null && alias.getAliasName() != null) {
                            ...
                            TObjectName viewColumnName = parser.parseObjectName(
                                    generateQuotedName(parser,
            ...
            if (sourceElement instanceof ResultColumn) {
                ...
                if (!sourceColumn.getStarLinkColumns().isEmpty()) {
                    for (int k = 0; k < sourceColumn.getStarLinkColumns()
                        ...
                        if (relation
                                .getRelationType() == RelationType.fdd) {
    ...
    private void appendResultSets(Document doc, Element dlineageResult) {
        List<TResultColumnList> selectResultSets = modelManager
                .getSelectResultSets();
        List<TTable> tableWithSelectSetResultSets = modelManager
                .getTableWithSelectSetResultSets();
        List<TSelectSqlStatement> selectSetResultSets = modelManager
                .getSelectSetResultSets();
        List<TCTE> ctes = modelManager.getCTEs();
        List<TParseTreeNode> mergeResultSets = modelManager
                .getMergeResultSets();
        List<TParseTreeNode> updateResultSets = modelManager
                .getUpdateResultSets();
    ...
    private void appendViews(Document doc, Element dlineageResult) {
        ...
        for (int i = 0; i < views.size(); i++) {
            View viewModel = (View) modelManager
    ...
    private void analyzeSelectStmt(TSelectSqlStatement stmt) {
        if (stmt.getSetOperatorType() != ESetOperatorType.none) {
            ...
            if (stmt.getResultColumnList() != null) {
                Object queryModel = modelManager
    ...
    private void analyzeImpactRelation( TResultColumn column,
        ...
        relation.setTarget(new ResultColumnRelationElement(
                (ResultColumn) modelManager.getModel(column)));
        ...
        for (int j = 0; j < objectNames.size(); j++) {
            ...
            if (table != null) {
                if (modelManager.getModel(table) instanceof Table) {
                    Table tableModel = (Table) modelManager
                            .getModel(table);
                    if (tableModel != null) {
    ...
    private void analyzeRecordSetRelation( TResultColumn column,
        ...
        for (int i = 0; i < functions.size(); i++) {
            ...
            if (stmtStack.peek().getTables().size() == 1) {
                Object tableObject = modelManager.getModel(
                ...
                if (table != null) {
                    if (modelManager
                        ...
                        if (tableModel != null) {
                            ...
                            .getModel(table) instanceof QueryTable) {
                        QueryTable queryTable = (QueryTable) modelManager
                                .getModel(table);
                        ...
                        if (subquery != null
                            ...
                            if (columnName.getSourceColumn() != null) {
                                ...
                                Object tablModel = modelManager.getModel(
                                ...
                                if (table.getCTE() != null) {
                                    ...
                                    if (columnName.getSourceColumn() != null) {
                                        ...
                                        if (model instanceof ResultColumn) {
                                            ...
                                            .getSourceTable() != null) {
                                        Object tableModel = modelManager
                                                .getModel(columnName
                                                        .getSourceTable());
        ...
        if (columns != null) {
            for (int i = 0; i < columns.size(); i++) {
                ...
                for (int j = 0; j < objectNames.size(); j++) {
                    ...
                    TTable table = modelManager.getTable(stmt,
                            columnName);
                    if (table != null) {
                        if (modelManager
                                .getModel(table) instanceof Table) {
                            ...
                            for (int j = 0; j < rightObjectNames.size(); j++) {
                                ...
                                if (modelManager.getModel(
                                            ...
                                            .getSourceColumn( ) != null )
                                    {
                                        ...
                                        QueryTable table = (QueryTable) modelManager
                                                .getModel( leftTable );
                                        ResultColumn resultColumn = matchResultColumn(
                                                table.getColumns( ),
                                                leftObjectName );
                                        if ( resultColumn != null )
                                        {
                                ...
                                TObjectName rightObjectName = rightObjectNames
                                        .get(j);
                                TTable rightTable = modelManager
                                        .getTable(stmt, rightObjectName);
                                if (rightTable == null) {
                                    rightTable = rightObjectName
                                            .getSourceTable();
                                }
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            ...
            System.out.println(
                    "/log: Option, generate a fdd.log file to log information.");
            return;
        }
