...
namespace gudusoft.gsqlparser.test.formatsql
{
    ...
    [TestClass()]
    public class testCreateFunctionProcedure
	{
        [TestMethod()]
        public void testBEStyle_Function_leftBEOnNewline()
		{
			 ...
			 Assert.IsTrue(result.Trim().Equals("CREATE FUNCTION dbo.Fn_gettoporders\n" + "   (@custid AS INT,\n" + "    @n      AS INT,\n" + "    @test   AS CHAR\n" + ") \n" + "RETURNS TABLE \n" + "AS \n" + "   RETURN \n" + "      SELECT   TOP(@n) *\n" + "      FROM     sales.salesorderheader\n" + "      WHERE    customerid = @custid\n" + "      ORDER BY totaldue DESC", StringComparison.OrdinalIgnoreCase));
			// System.out.println(result);
		}

        [TestMethod()]
        public void testBEStyle_Function_rightBEOnNewline()
		{
			 GFmtOpt option = GFmtOptFactory.newInstance(this.GetType().Name+"."+ new System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name);
        ...
        [TestMethod()]
        public void testBEStyle_Function_FirstParamInNewline()
		{
			 GFmtOpt option = GFmtOptFactory.newInstance(this.GetType().Name+"."+ new System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name);
        ...
        [TestMethod()]
        public void testParameters_Comma()
		{
			 GFmtOpt option = GFmtOptFactory.newInstance(this.GetType().Name+"."+ new System.Diagnostics.StackTrace().GetFrame(0).GetMethod().Name);
			...
			option.beStyleFunctionRightBEOnNewline = false;
			 string result = FormatterFactory.pp(sqlparser, option);
			//System.out.println(result);
			Assert.IsTrue(result.Trim().Equals("CREATE FUNCTION dbo.Fn_gettoporders(@custid AS INT\n" + "                                    , @n    AS INT\n" + "                                    , @test AS CHAR) \n" + "RETURNS TABLE \n" + "AS \n" + "  RETURN \n" + "    SELECT   TOP(@n) *\n" + "    FROM     sales.salesorderheader\n" + "    WHERE    customerid = @custid\n" + "    ORDER BY totaldue DESC", StringComparison.OrdinalIgnoreCase));
		}
        ...
        public void testBigScript()
		{
			 ...
			 sqlparser.parse();
			 string result = FormatterFactory.pp(sqlparser, option);
			  Console.WriteLine("not formatted correctly");
			//Assert.IsTrue("not formatted correctly",false);
		}
