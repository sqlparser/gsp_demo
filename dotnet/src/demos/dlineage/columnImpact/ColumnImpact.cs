...
namespace demos.dlineage.columnImpact
{
    ...
    using DDLParser = demos.dlineage.metadata.DDLParser;
    using MetaDB = demos.dlineage.metadata.MetaDB;
    ...
    using AliasModel = demos.dlineage.model.view.AliasModel;
    using Clause = demos.dlineage.model.view.Clause;
    ...
    using FieldModel = demos.dlineage.model.view.FieldModel;
    using ReferenceModel = demos.dlineage.model.view.ReferenceModel;
    using TableModel = demos.dlineage.model.view.TableModel;
    ...
    using table = demos.dlineage.model.xml.table;
    ...
    public class ColumnImpact
    {
        ...
        private const String TABLE_CONSTANT = "CONSTANT";
        ...
        internal class columnsInExpr : IExpressionVisitor
        {
            ...
            public virtual bool exprVisit(TParseTreeNode pNode, bool isLeafNode)
            {
                ...
                else if (lcexpr.ExpressionType == EExpressionType.subquery_t)
                {
                    ...
                    for (int i = 0; i < lcexpr.SubQuery.ResultColumnList.Count; i++)
                    {
                        impact.linkFieldToTables(null, lcexpr.SubQuery.ResultColumnList.getResultColumn(i), lcexpr.SubQuery, level + 1, ClauseType.select);
                    }
        ...
        internal class TAlias
        {
            ...
            public virtual string Column
            {
                ...
                set
                {
                    ...
                    this.column = outerInstance.removeQuote(value);
                }
            }
            ...
            public virtual string Alias
            {
                ...
                set
                {
                    ...
                    this.alias = outerInstance.removeQuote(value);
                }
            }
        ...
        public ColumnImpact(Element fileNode, EDbVendor dbVendor, IDictionary<TableMetaData, IList<ColumnMetaData>> tableColumns, bool strict)
        {
            ...
            sqlparser.sqltext = SQLUtil.getFileContent(fileNode.Attribute("name").Value).ToUpper();
        ...
        public ColumnImpact(string sqlText, EDbVendor dbVendor, IDictionary<TableMetaData, IList<ColumnMetaData>> tableColumns, bool strict)
        {
            ...
            sqlparser.sqltext = sqlText.ToUpper();
            sqlparser.MetaDatabase = metaDB;
            this.metaDB = metaDB;
        }
        ...
        private TColumn attrToColumn(TExpression attr, TCustomSqlStatement stmt, ClauseType clauseType, TAlias parentAlias)
        {
            ...
            {
                if ((clauseType == ClauseType.select || clauseType == ClauseType.casewhen || clauseType == ClauseType.casethen) && parentAlias != null)
                {
                    ...
                    {
                        if (resultColumn.AliasClause != null && !parentAlias.AliasDisplayName.Equals(resultColumn.ColumnAlias, StringComparison.CurrentCultureIgnoreCase))
                        {
                            linkFieldToTables(parentAlias, resultColumn, stmt, 0, clauseType);
                        }
                        return null;
                    }
                }
            ...
            else
            {
                ...
                if (!containColumn)
                {
                    if (tables.Count > 0)
                    {
                        ...
                        {
                            column.tableNames.Add(TABLE_CONSTANT);
                            column.tableFullNames.Add(TABLE_CONSTANT);
                        }
                    }
                }
            }
        ...
        private TColumn attrToColumn(TObjectName objectName, TCustomSqlStatement stmt, ClauseType clauseType)
        {
            TColumn column = new TColumn();
            column.clauseType = clauseType;
            column.columnName = removeQuote(objectName.endToken.ToString());
            ...
            if (objectName.ToString().IndexOf(".", StringComparison.Ordinal) > 0)
            {
                column.columnPrex = removeQuote(objectName.ToString().Substring(0, objectName.ToString().LastIndexOf(".", StringComparison.Ordinal)));
        ...
        private bool findColumnInTables(TColumn column, string tableName, TCustomSqlStatement select, int level, ClauseType clause)
        {
            ...
            if (TABLE_CONSTANT.Equals(tableName))
            {
                    ...
                    case ETableSource.subquery:
                        for (int i = 0; i < column.tableNames.Count; i++)
                        {
                            ...
                            for (int j = 0; j < selectStat.tables.Count; j++)
                            {
                                ...
                                {
                                    if (getTableAliasName(selectStat.tables.getTable(j)).Equals(name, StringComparison.CurrentCultureIgnoreCase))
                                    {
                                        ret = findColumnInSubQuery(selectStat, column.columnName, level, column.location);
                                        flag = true;
                                        break;
                                ...
                                {
                                    if (selectStat.tables.getTable(j).TableName.ToString().Equals(name, StringComparison.CurrentCultureIgnoreCase))
                                    {
                                        ret = findColumnInSubQuery(selectStat, column.columnName, level, column.location);
                                        flag = true;
                                        break;
        ...
        private bool findColumnInTables(TColumn column, TCustomSqlStatement select, int level, string columnName, Tuple<long, long> originLocation)
        {
            ...
            {
                if (!string.ReferenceEquals(columnName, null) && metaDB != null)
                {
                    int dotIndex = tableName.LastIndexOf(".", StringComparison.Ordinal);
                    string TableOwner = null;
                    string tableRealName = null;
        ...
        private void findColumnsFromGroupBy(TCustomSqlStatement select, int level)
        {
            ...
            {
                for (int j = 0; j < ((TSelectSqlStatement)select).GroupByClause.Items.Count; j++)
                {
                    ...
                    if (sqlparser.sqlstatements.get(k) is TCustomSqlStatement)
                    {
                        ...
                        foreach (TAlias alias in aliases)
                        {
                            ...
                            if (dependMap.ContainsKey(alias.Alias))
                            {
                                ...
                                foreach (TResultEntry result in results)
                                {
                                    ...
                                    if (result.isConstant)
                                    {
                                        key = removeQuote((TABLE_CONSTANT.ToLower()
                                    ...
                                    else if ("*".Equals(result.targetColumn))
                                    {
                                        ...
                                        {
                                            key = removeQuote(result.targetTable.FullName.ToLower() + "." + result.columnObject.linkColumnName);
                                        }
                                        else
                                        {
                        ...
                        if (showUIInfo)
                        {
                            for (int i = 0; i < tableCollections.Count; i++)
                            {
                                ...
                                {
                                    element.Add(new XAttribute("name", TABLE_CONSTANT));
                                }
                                if (columnImpactResult != null)
                                {
        ...
        private TableMetaData getConstantTableMetaData()
        {
            ...
            tableMetaData.Name = (TABLE_CONSTANT);
            ...
            else if (select is TMergeSqlStatement)
            {
                ...
                for (int i = 0; i < merge.WhenClauses.Count; i++)
                {
                    ...
                    else if (whenClause.InsertClause != null)
                    {
                        ...
                        {
                            linkFieldToTables(whenClause.InsertClause.ColumnList.getObjectName(j), whenClause.InsertClause.Valuelist.getResultColumn(j), select, ClauseType.merge);
                        }
                    }
                    else if (whenClause.DeleteClause != null)
                    {
        ...
        private bool linkFieldToTables(TObjectName objectName, TResultColumn field, TCustomSqlStatement select, ClauseType clauseType)
        {
            TAlias alias = new TAlias(this);
            ...
            TColumn column = attrToColumn(objectName, select, clauseType);

            bool ret = findColumnInTables(column, select, 1, null, null);
            ...
            if (select is TMergeSqlStatement)
            {
                ...
                linkColumn.columnName = removeQuote(currentSource);
        ...
        private bool linkFieldToTables(TAlias parentAlias, TResultColumn field, TCustomSqlStatement select, int level, ClauseType clauseType)
        {
            ...
            if (level == 0)
            {
                ...
                if (isTopSelectStmt(select) && !ignoreTopSelect_Renamed)
                {
                    ...
                    if (parentAlias != null)
                    {
                        ...
                        column.columnName = removeQuote(currentSource);
                    }
                    else
                    {
            ...
            if (select.ParentStmt is TUpdateSqlStatement)
            {
                ...
                for (int j = 0; j < update.ResultColumnList.Count; j++)
                {
                    ...
                    TExpression expression = resultColumn.Expr.LeftOperand;
                    if (expression.ExpressionType == EExpressionType.list_t)
                    {
                        TExpressionList setExpression = resultColumn.Expr.LeftOperand.ExprList;

                        for (int k = 0; k < select.ResultColumnList.Count; k++)
                        {
                            if (field == select.ResultColumnList.getResultColumn(k))
                            {
